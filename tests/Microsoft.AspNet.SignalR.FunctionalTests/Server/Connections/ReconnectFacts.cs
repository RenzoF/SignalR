        ...
        public async Task GroupsWorkAfterServerRestart()
        {
            ...
            using (host)
            {
                using (var connection = CreateHubConnection("http://foo/"))
                {
                    ...
                    // MemoryHost doesn't support WebSockets and it is somewhat had to ensure
                    // and reconnected isn't reliably fired with the LongPollingTransport.
                    ...
                    Assert.True(await reconnectedEvent.WaitAsync(TimeSpan.FromSeconds(15)), "Timed out waiting for message.");
